...
public class Manager implements IWorker, InventoryListener {
    ...
    /**
     * The Manager inputs how much of an ingredient the Restaurant has received.
     * This gets printed to a new txt. The manager will compare between the request txt and receivedShipments
     * txt to see which shipments have been received. This is useful if a shipment has not come in, so the manager
     * can send an email to the supplier.
     * @param ingredient
     * @param amount
     */
    public void confirmReceived(String ingredient, int amount) {
        try (PrintWriter out =
                     new PrintWriter(new BufferedWriter(new FileWriter(receivedShipments)))) {
